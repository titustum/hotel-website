...
<div class="max-w-4xl mx-auto py-12 px-6">
    <div class="text-center mb-10 mt-12">
        <span class="text-xs font-bold tracking-[0.18em] uppercase text-amber-500 mb-3 block">Conference Booking</span>
        <h1 class="font-[Cormorant_Garamond] text-4xl md:text-5xl font-light text-navy">Reserve a Meeting Space</h1>
        <p class="text-gray-500 mt-4 max-w-lg mx-auto">Book our modern conference rooms for your next event, training,
            or meeting.</p>
    </div>
    ...
    <div class="bg-white shadow-sm rounded-2xl p-8 mb-8">
            ...
            <div>
                ...
                <select wire:model="conference_room_id"
                    ...
                    <option value="{{ $room->id }}">
                        {{ $room->name }} – KES {{ number_format($room->price_per_day) }}/day (max {{ $room->capacity }}
                        pax)
                    </option>
                    @endforeach
                </select>
        ...
        <div class="mt-4 p-3 rounded-lg {{ $isAvailable ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700' }}">
    ...
    <div class="bg-white rounded-2xl shadow-sm p-8 border-t-4 border-amber-400">
        ...
        <p class="mb-4 text-gray-600">You are booking <span class="font-semibold">{{ $selectedRoom->name }}</span> for
